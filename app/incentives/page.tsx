      ...
      <section className="py-20 max-w-5xl mx-auto px-4">
        {Object.entries(INCENTIVES_BY_STATE).map(([code, list]) => (
          <div key={code} className="mb-12">
            <h2 className="text-2xl font-bold text-[#32373c] mb-4">{STATE_NAMES[code]}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {list.map(inc => (
                <div key={inc.name} className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                  <div className="flex justify-between items-start mb-1">
                    <h3 className="font-bold text-[#32373c] text-sm">{inc.name}</h3>
                    <span className="text-xs font-bold text-[#c9a700] ml-2 whitespace-nowrap">{inc.amount}</span>
                  </div>
